...
v7: adds smart_chat, classify_query, plan_task, execute_step,
run_shell, web_fetch, notify, give_feedback, compact_memories,
conversation_hook, get_profile, update_profile, context_prefetch.
"""
...
async def search_memory(query: str, collections: list[str] | None = None, tags: list[str] | None = None, limit: int = 5, source_name: str | None = None) -> str:
    """Search the vector memory for relevant documents and memories."""
...
async def remember_memory(text: str, collection: str = "memory_personal", subject: str | None = None, tags: list[str] | None = None, source: str = "nanobot", summarize: bool = True) -> str:
    """Store a durable memory (preference, decision, fact) in the vector database."""
...
@mcp.tool()
async def smart_chat(messages: list[dict[str, str]], auto_classify: bool = True, auto_retrieve: bool = True, inject_profile: bool = True) -> str:
    """Smart chat: auto-classifies the query, retrieves relevant context, injects user profile, and routes to the optimal model."""
    return json.dumps(await _post("/smart-chat", {"messages": messages, "auto_classify": auto_classify, "auto_retrieve": auto_retrieve, "inject_profile": inject_profile}), ensure_ascii=False)
...
async def context_prefetch(query: str, limit: int = 5, inject_profile: bool = True) -> str:
    """Retrieve relevant context + user profile for system prompt injection before answering."""
...
@mcp.tool()
async def plan_task(query: str, context: str = "") -> str:
    """Decompose a complex task into a multi-step plan with tool calls."""
...
async def run_shell(command: str) -> str:
    """Run a pre-approved read-only shell command (systemctl status, journalctl, openssl, curl, dig, df, uptime)."""
